...
Please note that current configuration creates worker nodes where the security group rule allows egress to all public internet addresses. If this does not satisfy your requirements, in main.tf modify the value for egress_cidr_blocks, or find other ways to limit connectivity or other ways to create desired clusters.
...
If you like to use an existing VPC, then in main.tf configure ```use_existing_vpc = true```, and configure existing_vpc_id and existing_private_subnet_ids. This will in existing VPC with two private subnets, create EKS control plane and worker nodes.
...
The module configuration will create compute-intensive worker nodes with activated CPU pinning (by having Kubelet configured with static CPU policy), and install NFD. If you don't like CPU pinning, in main.tf configure ```create_compute-intensive = false```.
...
Each created worker node will have labels as per [eks.tf](./eks.tf), and optionally additional ones added by NFD. Those labels can be used for pod scheduling with pod configurations including ```nodeSelector:``` fields like explained in Kubernetes Docs [here](https://kubernetes.io/docs/tasks/configure-pod-container/assign-pods-nodes/#create-a-pod-that-gets-scheduled-to-your-chosen-node).
